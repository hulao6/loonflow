...
export interface IWorkflowComponent {
  ...
  value?: any,
  description: string,
  type: 'text' | 'textarea' | 'number' | 'select' | 'radio' | 'checkbox' | 'time' | 'date' | 'datetime' | 'user' | 'department' |
  'file' | 'link' | 'richtext' | 'richText' | 'externaldata' | 'creator_info' | 'created_at' | 'act_state' | 'workflow_info' |
  'current_assignee_infos' | 'title' | 'ticket_node_infos',
  layout: ILayout,
  label: ILabel,
  props: IProps,
}
...
export interface IWorkflowMetadata {
  ...
  versionName: string,
  description: string,
  /** Resolved title for ticket detail header; from label.ticketDetailTitleTemplate or default "{name} - {versionName}" */
